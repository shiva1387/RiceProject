...
# Author(s): Shiv
# Version: 23082016
# Input: ".tsv" file from XCMS Metabolomics data (data matrix generated by Shiv on NERI computer)
# Software: XCMS
...
setwd('/Users/shiv/Dropbox-lsius/Dropbox/Projects/NUS/RiceProject/Metabolomics/data/')

### RUN THE USER CREATED FUNCTIONS FIRST

# For AR 2 week dataset generated by Shiv  in xcms
mzfilename<-"AR_230816_xcms1_48.tsv"
...
SampleGroups<-sapply(names(ms_data_samples), function(x) paste(strsplit(x,"")[[1]][1:2],collapse = ""))
SampleGroups<-as.vector(SampleGroups)

SampleGroups= stri_replace_all_fixed(SampleGroups,c('E1','E2','E3','E4'), c('EX'), vectorize_all=FALSE) #replacing group name as EC
...
SampleGroups_rna_wo= stri_replace_all_fixed(SampleGroups_rna_wo,c('E1','E2','E3','E4'), c('EX'), vectorize_all=FALSE) #replacing group name as EC
...
pdf("mzrt_AR_Aug2016.pdf", height=12, width=12)
...
forPlot<-data.frame(PCaxisA = ms_data_princomp$loadings[,1],PCaxisB = ms_data_princomp$loadings[,2], 
                    dataType=SampleGroups_rna) #Subset SampleGroups as the last 9 are blanks #SampleGroups
...
ggsave("pca_pc12_samples_AR_Aug2016_metab.pdf",useDingbats=FALSE, h=4,w=6,units="cm",scale = 2)
...
###Analyzing sample correlations
forPlot_c<-cor(ms_data_samples_rna_norm)
pdf("Correlations_samples.pdf")
corrplot(forPlot_c, method="number")
dev.off()

###Plotting dendrograms

forHist_dend<-t(ms_data_samples_rna_norm) %>% dist(method = "euclidean") %>% hclust(method = "average") %>% as.dendrogram()
pdf("Dendrogram_euclidean_samples.pdf")
plot(forHist_dend)
dev.off()
...
png("pvalue_rna_metabolomics_AR_Aug2016.png")
hist(ms_data_samples_rna_perm_pval$rawp,xlab=paste0("Diff features (p<0.05) = ",nrow(ms_data_samples_rna_perm_matrix)))
dev.off()

## Wilcox test
# Comparing WT with MU
pval.list <- apply(ms_data_samples_rna_norm,1,function(x){t.test(x[9:16],x[17:24],var.equal = FALSE,paired=FALSE)$p.value})
...
WTvsMU_foldchange<-apply(metab.sig_WTvsMU,1,function(x){foldchange(mean(exp(x[9:16])-1),mean(exp(x[17:24])-1))}) 
...
write.table(WTvsMU_mass.list,"differential_AR_WTvsMU_massFeatures.txt",sep="\t",quote=FALSE)
write.table(metab.sig_WTvsMU,"differential_AR_WTvsMU.txt",quote=FALSE,row.names=FALSE,sep="\t")
#Updated on 20 Jan 2016 to obtain the format required for MatchCompoundIdKEGG.R
write.table(WTvsMU_sig,"differential_AR_WTvsMU_forId.txt",quote=FALSE,sep="\t",col.names=NA)

# Comparing WT with EC
pval.list <- apply(ms_data_samples_rna_norm,1,function(x){t.test(x[1:8],x[17:24],var.equal = FALSE,paired=FALSE)$p.value})
pval.adj.list <- p.adjust(pval.list,method="BH")
id.sig <- which(pval.adj.list < 0.05 )
...
WTvsEC_pvalues<-pval.list[id.sig]
#WTvsEC_foldchange<-apply(metab.sig_WTvsEC,1,function(x){foldchange(mean(x[17:24]),mean(x[1:8]))}) #computing WT/EC
#Changed on 12-Aug to compute MU/WT
WTvsEC_foldchange<-apply(metab.sig_WTvsEC,1,function(x){foldchange(mean(exp(x[1:8])-1),mean(exp(x[17:24])-1))}) #computing WT/EC
...
mass.list<-cbind(metab.sig_WTvsEC,round(as.numeric(WTvsEC_pvalues),5),
                 round(as.numeric(WTvsEC_foldchange),5))
colnames(mass.list)[(dim(mass.list)[2]-1):dim(mass.list)[2]]<-c("P-VALUE","FoldChange(EC/WT)")

#FDR<0.05 and abs(PVAL) >2
WTvsEC_sig<-data.frame(mass.list[which(abs(mass.list[,c("FoldChange(EC/WT)")])>2),])
nrow(WTvsEC_sig)
#854

nrow(WTvsEC_sig[WTvsEC_sig$FoldChange.EC.WT>=2,])
#Up-regulated in EC 
#458
nrow(WTvsEC_sig[WTvsEC_sig$FoldChange.EC.WT<=-2,])
...
write.table(WTvsEC_mass.list,"differential_AR_WTvMU_massFeatures.txt",sep="\t",quote=FALSE)
write.table(metab.sig_WTvsEC,"differential_AR_WTvsEC.txt",quote=FALSE,row.names=FALSE,sep="\t")
#Updated on 20 Jan 2016 to obtain the format required for MatchCompoundIdKEGG.R
write.table(WTvsEC_sig,"differential_AR_WTvsEC_forId.txt",quote=FALSE,sep="\t",col.names=NA)

# #### Plotting venn diagram of differential features
venn.plot<-venn.diagram(list(as.character(rownames(WTvsEC_sig)),as.character(rownames(WTvsMU_sig))), filename=NULL, margin=0.1, scaled=TRUE, fill=c("red", "green"),
                        alpha=c(0.5,0.5), cex = 2, cat.dist=0.1, cat.fontface=4, category.names=c("WT vs EX", "WT vs Mutant"))
pdf("DiffentialFeatures_pval05Fc2_AR.pdf")
grid.draw(venn.plot)
dev.off()
...
WTvsEC_sig$mz.rt<-as.character(rownames(WTvsEC_sig))
WTvsMU_sig$mz.rt<-as.character(rownames(WTvsMU_sig))

merge_WTECMU<-merge(WTvsEC_sig,WTvsMU_sig,by="mz.rt", all=FALSE)
rownames(merge_WTECMU)<-merge_WTECMU$mz.rt
...
upWTEC_downWTMU<-merge_WTECMU[merge_WTECMU$FoldChange.EC.WT>=2 & merge_WTECMU$FoldChange.MU.WT<=2,]
...
downWTEC_upWTMU<-merge_WTECMU[merge_WTECMU$FoldChange.EC.WT<=2 & merge_WTECMU$FoldChange.MU.WT>=2,]
...
InverseTrends<-InverseTrends[,-grep(".y",colnames(InverseTrends))]
InverseTrends<-subset(InverseTrends, select=-c(mz.rt,P.VALUE.x,FoldChange.EC.WT.,FoldChange.MU.WT.))
colnames(InverseTrends)<-gsub(".x","",colnames(InverseTrends))
...
pheatmap(InverseTrends, scale="row",cluster_rows = TRUE, cluster_cols = FALSE,
...
upWTEC_upWTMU<-merge_WTECMU[merge_WTECMU$FoldChange.EC.WT>=2 & merge_WTECMU$FoldChange.MU.WT>=2,]
nrow(upWTEC_upWTMU)
#73
...
downWTEC_downWTMU<-merge_WTECMU[merge_WTECMU$FoldChange.EC.WT<=2 & merge_WTECMU$FoldChange.MU.WT<=2,]
